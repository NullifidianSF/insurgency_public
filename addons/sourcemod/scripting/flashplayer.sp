...
void BlindTarget(int client, float fDuration = 14.0) {
	if (!IsClientInGame(client)
	 ...
	 || GetEntPropFloat(client, Prop_Send, "m_flFlashDuration") != 0.0) {
		return;
	}
	...
	SetEntProp(client, Prop_Send, "m_iPlayerFlags", iPlayerFlags + 16);
	...
	CreateTimer(fDuration, Timer_ClearFlash, client, TIMER_FLAG_NO_MAPCHANGE);
}

Action Timer_ClearFlash(Handle timer, int client) {
